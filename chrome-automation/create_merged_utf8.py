...
with codecs.open('la_gang_territories.kml', 'r', encoding='utf-8') as f:
    content = f.read()
...
with codecs.open('la_gangs_central_east_utf8.kml', 'w', encoding='utf-8') as f:
    f.write(kml1)
print(f"  Total: {len(all_pms)} -> la_gangs_central_east_utf8.kml\n")

# Merge 2: Inglewood + Compton
print("Creating: Inglewood and Compton")
...
kml2 = header + '''    <Folder>
      <name>Inglewood and Compton</name>
'''
for pm in all_pms2:
    kml2 += pm + '\n'
kml2 += '''    </Folder>
...
with codecs.open('la_gangs_inglewood_compton_utf8.kml', 'w', encoding='utf-8') as f:
    f.write(kml2)
print(f"  Total: {len(all_pms2)} -> la_gangs_inglewood_compton_utf8.kml")

# Validate
print("\nValidating XML...")
import xml.etree.ElementTree as ET
for f in ['la_gangs_central_east_utf8.kml', 'la_gangs_inglewood_compton_utf8.kml']:
    try:
        tree = ET.parse(f)
        print(f"  {f}: Valid ✓")
